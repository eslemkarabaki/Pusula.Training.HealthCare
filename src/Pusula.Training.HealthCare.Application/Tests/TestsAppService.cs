...
#region GetList
    ...
    {
        var totalCount = await testRepository.GetCountAsync(
            input.FilterText, input.Code, input.Name, input.TestGroupId
        );
        var items = await testRepository.GetListAsync(
            input.FilterText, input.Code, input.Name, input.TestGroupId, input.Sorting, input.MaxResultCount,
            input.SkipCount
        );
...
#region Create
    ...
    {
        var test = await testManager.CreateAsync(input.Code, input.Name, input.TestGroupId, input.TestTypeId);
        return ObjectMapper.Map<Test, TestDto>(test);
    }
...
#region Update
    ...
    {
        var test = await testManager.UpdateAsync(
            id, input.Code, input.Name, input.TestGroupId, input.TestTypeId, input.ConcurrencyStamp
        );
        return ObjectMapper.Map<Test, TestDto>(test);
    }
...
#region Delete
    ...
    public virtual async Task DeleteAsync(Guid id) => await testRepository.DeleteAsync(id);

    [Authorize(HealthCarePermissions.Tests.Delete)]
    public virtual async Task DeleteByIdsAsync(List<Guid> testIds) => await testRepository.DeleteManyAsync(testIds);
...
#region Excel
    ...
    public virtual async Task<IRemoteStreamContent> GetListAsExcelFileAsync(TestExcelDownloadDto input)
    {
        ...
        return new RemoteStreamContent(
            memoryStream, $"Tests_{DateTime.Now:yyyy-MM-dd_HH-mm}.xlsx",
